...
public class ChessMatch
{
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition)
	{
		...
		if(movedPiece instanceof Pawn) {
			if((movedPiece.getColor() == Color.WHITE && target.getRow() == 0) || (movedPiece.getColor() == Color.BLACK && target.getRow() == 7)) {
				...
				promoted = replacePromotodePiece("Q");
			}
		}
	...
	public ChessPiece replacePromotodePiece(String type) {
		if (promoted == null) {
	...
	private boolean testCheckMate(Color color)
	{
		...
		List<Piece> allPiecesThisColor = piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == color).collect(Collectors.toList());
		for (Piece piece : allPiecesThisColor)
		{
			boolean[][] matrix = piece.possibleMoves();
			for (int i = 0; i < board.getRows(); i++)
			{
